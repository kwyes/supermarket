...
function page_navigation(page) {
	location.href = "village.php?page=" + page;
}

function page_navigation_search(page) {
	var search_key = document.getElementsByName("search_key")[0].value;
	location.href = "village.php?mode=search&search_key=" + encodeURI(search_key) + "&page=" + page;
}
</script>
...
if($mode == "search") {
	...
	$boardVillage_query = "SELECT TOP ".$per_page." seq, subject, content, CONVERT(char(10), upload_date, 126) AS upload_date ".
						  "FROM new_board ".
				...
				<? if($page == 1) { ?>
					<? while($premium_row = mssql_fetch_array($premium_query_result)) { ?>
						<tr class="table_premium">
							...
							<td class="table_subject"><a href="village_view.php?seq=<?=$premium_row['seq']; ?>" class="post"><?=Br_iconv($premium_row['subject']); ?></a></td>
				...
				<? } else { ?>
					...
					<? while($boardVillage_row = mssql_fetch_array($boardVillage_query_result)) { ?>
						<tr class="table_list <?=(($i++ == $boardVillage_num_row) ? 'table_last' : '' ); ?>">
							...
							<td class="table_subject"><a href="village_view.php?seq=<?=$boardVillage_row['seq']; ?>" class="post"><?=Br_iconv($boardVillage_row['subject']); ?></a></td>
