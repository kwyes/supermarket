...
function page_navigation(page) {
	location.href = "?menu=menu4&list=news&page=" + page;
}

function page_navigation_search(page) {
	var search_key = document.getElementsByName("search_key")[0].value;
	location.href = "?menu=menu4&list=news&mode=search&search_key=" + encodeURI(search_key) + "&page=" + page;
}
</script>
...
if($mode == "search") {
	...
	$boardNews_query = "SELECT TOP ".$per_page." seq, subject, content, CONVERT(char(10), upload_date, 126) AS upload_date, click_counter, active ".
						  "FROM new_board ".
...
<div class="content_wrapper">
		...
		<? if($page == 1) { ?>
			<? while($premium_row = mssql_fetch_array($premium_query_result)) { ?>
				<tr class="table_premium">
					...
					<td class="table_subject"><a href="?menu=menu4&list=news_write&seq=<?=$premium_row['seq']; ?>" class="post"><?=Br_iconv($premium_row['subject']); ?></a></td>
		...
		<? } else { ?>
			...
			<? while($boardNews_row = mssql_fetch_array($boardNews_query_result)) { ?>
				<tr class="table_list <?=(($i++ == $boardNews_num_row) ? 'table_last' : '' ); ?>">
					...
					<td class="table_subject"><a href="?menu=menu4&list=news_write&seq=<?=$boardNews_row['seq']; ?>" class="post"><?=Br_iconv($boardNews_row['subject']); ?></a></td>
					<td><?=$boardNews_row['upload_date']; ?></td>
					<td><?=$boardNews_row['click_counter']; ?></td>
					<td><span style="color:red; font-weight:bold;"><?=(($boardNews_row['active'] == 0) ? 'X' : 'O' ); ?></span></td>
				</tr>
			<? } ?>
		<? } ?>
	</table>
	...
	<div style="margin-top:30px;">
		...
		<div style="float:right;"><input type="button" class="btn" onClick="location.href='?menu=menu4&list=news_write'" value="글쓰기"></div>
	</div>
</div>