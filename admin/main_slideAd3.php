			...
			if($_FILES['reserveAd_image']['error'][$i] == 0) {
				...
				if(in_array($ext, $ableExt)) {
					...
					if(in_array($_FILES['reserveAd_image']['type'][$i], $ableImage)) {
						...
						$reserveAdAddImg_query = "UPDATE new_mainPage SET subject = '1', content = '$reserveAd_date[$i]', filename = '$new_filename' WHERE type = 1 AND seq = ".($i + 4);
...
<form name="form_slideAd" action="?menu=main&list=slideAd3" enctype="multipart/form-data" method="post" accept-charset="utf-8">
...
function toggle_reserveDate(toggle, seq) {
	if(toggle == "enable")
		document.getElementsByName("slideAd_reserve_date_" + seq)[0].disabled = false;
	if(toggle == "disable")
		document.getElementsByName("slideAd_reserve_date_" + seq)[0].disabled = true;
}
...
if($mode == "save") {
	...
	if($slideAd_delImg) {
		for($i = 0; $i < strlen($slideAd_delImg); $i++) {
			...
			$slideAdDelImg_query = "UPDATE new_mainPage SET filename = '' WHERE type = 1 AND seq = ".(int)$slideAd_delImg[$i];
			mssql_query($slideAdDelImg_query, $conn_hannam);
		}
	}

	if($_FILES['slideAd_image']) {
		$slideAdGetSeq_query = "SELECT seq FROM new_mainPage WHERE type = 1 AND filename = '' ORDER BY seq";
			...
			if($_FILES['slideAd_image']['error'][$i] == 0) {
				...
				if(in_array($ext, $ableExt)) {
					...
					if(in_array($_FILES['slideAd_image']['type'][$i], $ableImage)) {
						...
						$slideAdAddImg_query = "UPDATE new_mainPage SET filename = '$new_filename' WHERE type = 1 AND seq = ".$add_image[$i];
						mssql_query($slideAdAddImg_query, $conn_hannam);
						move_uploaded_file($_FILES['slideAd_image']['tmp_name'][$i], $new_fullpath);
					}
				}
				...
				<td>
					<table width="100%">
						...
						<tr>
							...
							<td>
								<input type="radio" name="slideAd_date_type_<?=$slideAd_row['seq']; ?>" onChange="toggle_reserveDate('disable', <?=$slideAd_row['seq']; ?>)" value="1" checked>즉시변경
								<input type="radio" name="slideAd_date_type_<?=$slideAd_row['seq']; ?>" onChange="toggle_reserveDate('enable', <?=$slideAd_row['seq']; ?>)" value="2">예약변경
								<input type="text" name="slideAd_reserve_date_<?=$slideAd_row['seq']; ?>" value="<?=date("Y-m-d 21:50:00")?>" disabled>
							</td>
						</tr>
						-->
						<tr>
