<?php
	 ...
	 * @since Create: 2012. 06. 28 / Update: 2012. 08. 17
	 */  

	include_once("./include_setup.php");
	/*
	// Input variable
	$tel = JMC_GetInput("tel", "GET");
	$session = JMC_GetInput("session", "GET");
	$store = (JMC_GetInput("store", "GET")) ? JMC_GetInput("store", "GET") : "bby"; //Default: bby
	$language = (JMC_GetInput("language", "GET")) ? JMC_GetInput("language", "GET") : "korean"; //Default: korean
	$search_mode =  JMC_GetInput("search_mode", "GET");
	$search_keyword = JMC_GetInput("search_keyword", "GET");
	$search_index =  JMC_GetInput("search_index", "GET");
	$search_plucategory = JMC_GetInput("search_plucategory", "GET");
	$search_onsale = JMC_GetInput("search_onsale", "GET");
	$search_store = JMC_GetInput("search_store", "GET");
	$boardId = JMC_GetInput("boardId", "GET");
	$page = (JMC_GetInput("page", "GET")) ? JMC_GetInput("page", "GET") : "1"; //Default: 1
	$mode = JMC_GetInput("mode", "GET");
	*/
	// Input variable
		...
		try {
			...
			if($result['session_id'] == $session) {
				...
				if($search_keyword)
				{
					...
					{
						/* JMC
						if($_GET['search_from'] != "mainpage")
						{
							//검색 키워드 저장
							$BoardData->InsertSearchKeyword('', $search_keyword, $today);
							//키워드 삭제(한달 전꺼)
							$BoardData->DeleteSearchKeyword($today);
						}
						*/
				...
				*/
				...
				$IT_where = " prodId != '' $plc_where $where_restricted $where_onsale and useYN = 'Y' and prodBal < 0 and LastModDate > '$PluTerm' ".$where_keyword." and webview is null";
				$IT_field_distinct = "distinct(prodName), prodId, prodKname, prodType, prodOUprice, prodBal, prodTax, prodUnit, prodPromo, promoPrice, LastModDate, LastModTime, useYN, convert(varchar(20), promoSdate, 120) as promoSdate, convert(varchar(20), promoEdate, 120) as promoEdate";
				$IT_field = "prodId, prodName, prodKname, prodType, prodOUprice, prodBal, prodTax, prodUnit, prodPromo, promoPrice, LastModDate, LastModTime, useYN, convert(varchar(20), promoSdate, 120) as promoSdate, convert(varchar(20), promoEdate, 120) as promoEdate";
				
				//상품 전체 수 구하는 쿼리
				$query = "select count(prodId) as row from mfProd where $IT_where";
				...
				$distint_subquery = "(select $IT_field_distinct from mfProd) SP";
				$query = "select top $IT_top $IT_field from mfProd where prodId not in (select top $cpage_que prodId from mfProd where $IT_where $IT_sort) and $IT_where $IT_sort";
